...
class MaskEliminator:
    def __init__(self):
        self.graph = tf.Graph()
        with self.graph.as_default():
            with open(get_model_file("hifill.pb", "https://github.com/Atlas200dk/sample-imageinpainting-HiFill/raw/master/GPU_CPU/pb/hifill.pb"), "rb") as f:
                output_graph_def = tf.GraphDef()
                output_graph_def.ParseFromString(f.read())
                tf.import_graph_def(output_graph_def, name="")

                self.output_graph_def = output_graph_def

            with tf.Session() as sess:
                init = tf.global_variables_initializer()
                sess.run(init)
    
    def eliminate_by_mask(self, main_image, mask_image):
        with self.graph.as_default():
        # with tf.Graph().as_default():
            # with open(os.path.join(os.path.dirname(__file__), 'pb/hifill.pb'), "rb") as f:
            #     output_graph_def = tf.GraphDef()
            #     output_graph_def.ParseFromString(f.read())
            #     tf.import_graph_def(output_graph_def, name="")
            ...
            with tf.Session() as sess:
                ...
                inpainted = inpaint(main_image, mask_image, sess, inpainted_512_node, attention_node, mask_512_node, image_ph, mask_ph, 6)
